...
loginForm.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        const idToken = await user.getIdToken(true); // force-refresh so claims are current
        ...
        if (!contentType.includes("application/json")) {
            throw new Error("Backend returned an unexpected response. Check the Node.js server.");
        }

        const result = await response.json();

        // Allow any known role — not just admin
        if (!response.ok) {
            // If the /api/admin-test endpoint rejects non-admin, call /api/me instead
            // Try reading the role from the Firebase ID token claims directly
